...
- The coefficient for Price is -0.0545, which means that for a one unit increase in Price, Sales decrease by 0.0545 units. The coefficient for UrbanYes is -0.0219, which means that for a store in an urban area, Sales decrease by 0.0219 units. The coefficient for USYes is 1.2006, which means that for a store in the US, Sales increase by 1.2006 units. 
...
- The p-values for Price and USYes are less than 0.05, so the null hypothesis can be rejected for these predictors. The p-value for UrbanYes is greater than 0.05, so the null hypothesis cannot be rejected for this predictor.
...
- The R-squared value for the model in (a) is 0.2393, and the R-squared value for the model in (e) is 0.2393. This means that the models in (a) and (e) explain 23.93% of the variance in Sales. The models in (a) and (e) fit the data equally well.
...
- There are no high leverage observations in the model from (e). There are a few outliers, but they do not appear to have a significant impact on the model.
...
- The p-value for the quadratic term is less than 0.05, so the null hypothesis can be rejected. This indicates that the quadratic term improves the model fit. The R-squared value for the model with the quadratic term is 0.2532, which is slightly higher than the R-squared value for the model without the quadratic term. This suggests that the quadratic term adds some explanatory power to the model i.e the evidence suggest that the quadratic term improves the model fit.
...
- The model obtained is $$\hat{y} = -1.0186 + 0.5145x$$. The values of $\hat{\beta}_0$ and $\hat{\beta}_1$ are -1.0186 and 0.5145, respectively. These values are close to the true values of $\beta_0$ and $\beta_1$. The model fits the data pretty well, with an R-squared value of 0.2522. The p-values for both $\hat{\beta}_0$ and $\hat{\beta}_1$ are less than 0.05, so the null hypothesis can be rejected for both predictors. The scatterplot shows a positive linear relationship between x and y. As x increases, y also increases. There is less noise in the data compared to the previous model, but the relationship is still clear.
